  ...
  for (unsigned int f = 0; f < GeometryInfo<2>::faces_per_cell; ++f)
    {
      ...
      if (dist > std::numeric_limits<double>::epsilon())
        {
          ...
          for (unsigned int q = 0; q < theta_q_points.size(); ++q)
            {
              std::cout << theta_q_points[q](0) << std::endl;
          ...
          for (unsigned int q = 0; q < theta_q_points.size(); ++q)
            {
              ...
              double p_1 = (Jk0[q] * B[q] + Jk1[q] * A[q]) / q_A[q];

              // r,i expansion
              // verified against R/r
              Tensor<1, 3> d_0 = A[q] / q_A[q];
              //              Tensor<1, 3> d_1 = B[q] / q_A[q] - A[q] / pow(q_A[q], 3) * q_C[q];
              ...
              Tensor<1, 3> b_0 = -Jk0[q];
              Tensor<1, 3> b_1 = 3 * g_1 - Jk1[q];

              // 1/r^3  expansion
              // verified against 1/pow(r,3.0)
              double s_3 = 1 / pow(q_A[q], 3.0);
              double s_2 = q_C[q] / pow(q_A[q], 5.0);

              // -1/r^3 * (3*r,i (r,k  Jk) - Ji ) expansion
              Tensor<1, 3> FF_2 = -(s_3 * b_0);
              Tensor<1, 3> FF_1 = -(s_2 * b_0 + s_3 * b_1);
              ...
              I_11 += FF_1 * log(fabs(q_rho[q] / beta));
              I_12 += log(fabs(q_rho[q] / beta));
              I_13 += uv_q_weights[q];
      ...
      //    std::cout << std::endl;
      std::cout << "I_11 = " << I_11[2] / (4.0 * numbers::PI) << std::endl;
