...
template <>
Tensor<1, 2>
SingularKernelIntegral<2>::evaluate_Vk_integrals()
{
  ExcNotImplemented();
  Tensor<1, 2> dummy;
  return dummy;
...
template <>
Tensor<1, 3>
SingularKernelIntegral<3>::evaluate_Vk_integrals()
{
  Tensor<1, 3> I_0;
  Tensor<1, 3> I_1;
  Tensor<1, 3> I_2;
          ...
          for (unsigned int q = 0; q < theta_q_points.size(); ++q)
            {
              ...
              Tensor<1, 3> F_1 =
                -3 * q_C[q] * Jk0[q] / pow(q_A[q], 5) -
                3 * A[q] / pow(q_A[q], 5) * (Jk0[q] * B[q] + Jk1[q] * A[q]) +
                Jk1[q] / pow(q_A[q], 3);
              // std::cout<<"f: "<<f<<"  q: "<<q<<" theta:
              // "<<theta_q_points[q](0)*180/dealii::numbers::PI<<"  F_1:
              // "<<F_1<<"  F_2: "<<F_2<<"  rho: "<<q_rho[q]<<"  beta:
              // "<<beta<<" gamma: "<<gamma<<"  w: "<<uv_q_weights[q]<<"  Jk0:
              // "<<Jk0[q]<<" Jk1: "<<Jk1[q]<<"  A: "<<A[q]<<"  B:
              // "<<B[q]<<std::endl; and the integral contribution of the
              // present quadrature node is here added
              I_2 +=
                -F_2 * (gamma / pow(beta, 2) + 1 / q_rho[q]) * uv_q_weights[q];
              ...
              for (unsigned int p = 0; p < rho_q_points.size(); ++p)
                {
                  ...
                  Tensor<1, 3> arg = fun * rho * inner_face_fe_values.JxW(p) -
                                     F_2 / pow(rho, 2) - F_1 / rho;

                  //                  std::cout<<"rho: "<<rho<<"  r: "<<r<<"
                  //                  arg1:
                  //                  "<<1/pow(r,3)*rho*inner_face_fe_values.JxW(p)<<
                  //                  "   arg2: "<<-F_2/pow(rho,2)-F_1/rho<<"
                  //                  diff:
                  ...
                  // we add the conribution of the present node to the surface
                  // integral the inner weights are given by rho_fe_v.JxW(p) but
                  // the jacobian is given by the scaling factor rho_jac_fact
                  I_0 +=
                    arg * rho_jac_fact * rho_fe_v.JxW(p) * theta_fe_v.JxW(q);
                }
            }
          /*
...
template <>
double
SingularKernelIntegral<2>::evaluate_integral()
{
  ExcNotImplemented();
  return 0.0;
}
...
template <>
double
SingularKernelIntegral<3>::evaluate_integral()
{
  double I_0 = 0.0;
  double I_1 = 0.0;
  double I_2 = 0.0;
  ...
  for (unsigned int f = 0; f < GeometryInfo<2>::faces_per_cell; ++f)
    {
      ...
      if (theta_1 < theta_0)
        theta_0 -= 2 * dealii::numbers::PI;
      // std::cout<<f<<" ->    theta_0: "<<theta_0*180/dealii::numbers::PI<<"
      // theta_1: "<<theta_1*180/dealii::numbers::PI<<std::endl;
      ...
      // we create a 1D quadature with the user selected number of quadrature
      // nodes
      QGauss<1> theta_quadrature(theta_quadrature_order);
      ...
      // because we also will need to carry out the surface integral in polar
      // coordinates for I_0, we will also need a 1D triangulation for rho
      Triangulation<1> rho_triangulation;
      ...
      rho_fe_v.reinit(rho_cell);
      // we get the quadrature points for rho (remember that they go from 0 to
      // 1)
      const std::vector<Point<1>> &rho_q_points =
        rho_fe_v.get_quadrature_points();
      ...
      const std::vector<Point<1>> &theta_q_points =
        theta_fe_v.get_quadrature_points();
      // we want to create a vector with the \hat(rho) values corresponding to
      // the face boundary at each theta value
      std::vector<double> q_rho(theta_q_points.size());
      // using deal.ii numbering, these lines do the trick
      for (unsigned int q = 0; q < theta_q_points.size(); ++q)
        {
      ...
      // we now create a set of vectors that will contain useful theta dependent
      // coefficients for each value of theta on the face f
      std::vector<Tensor<1, 3>> A(theta_q_points.size());
      std::vector<Tensor<1, 3>> B(theta_q_points.size());
      // values of A.norm()
      std::vector<double> q_A(theta_q_points.size());
          ...
          for (unsigned int i = 0; i < 3; ++i)
            {
              ...
              B[q][i] = eta_jacobian_grad[i][0][0] *
                          pow(cos(theta_q_points[q](0)), 2) / 2 +
                        eta_jacobian_grad[i][0][1] * cos(theta_q_points[q](0)) *
                          sin(theta_q_points[q](0)) +
                        eta_jacobian_grad[i][1][1] *
                          pow(sin(theta_q_points[q](0)), 2) / 2;
            }
      ...
      for (unsigned int q = 0; q < theta_q_points.size(); ++q)
        {
          ...
          double F_1 =
            -3 * q_C[q] * Jk0[q][2] / pow(q_A[q], 5) -
            3 * A[q][2] / pow(q_A[q], 5) * (Jk0[q] * B[q] + Jk1[q] * A[q]) +
            Jk1[q][2] / pow(q_A[q], 3);
          // std::cout<<"f: "<<f<<"  q: "<<q<<" theta:
          // "<<theta_q_points[q](0)*180/dealii::numbers::PI<<"  F_1: "<<F_1<<"
          // F_2: "<<F_2<<"  rho: "<<q_rho[q]<<"  beta: "<<beta<<"  gamma:
          // "<<gamma<<"  w: "<<uv_q_weights[q]<<"  Jk0: "<<Jk0[q]<<"  Jk1:
          // "<<Jk1[q]<<"  A: "<<A[q]<<"  B: "<<B[q]<<std::endl;
          // and the integral contribution of the present quadrature node is
          // here added
          ...
          // we must again create a set of quadrature points and weights to be
          // passed to a FEValues<2,3> to take care of the mapping to the 3D
          // space

          // we only want the mapping jacobian from such a FEValues, so we set
          ...
          for (unsigned int p = 0; p < rho_q_points.size(); ++p)
            {
              // based on rho and theta, the quadrature points are generated in
              // polar coordinates centered at the singularity eta_q_points[0]
              double rho = rho_jac_fact * rho_q_points[p](0);
              inner_uv_q_points[p] =
                Point<2>(rho * cos(theta_q_points[q](0)) + eta_q_points[0](0),
                         rho * sin(theta_q_points[q](0)) + eta_q_points[0](1));
            }
          ...
          for (unsigned int p = 0; p < rho_q_points.size(); ++p)
            {
              // this is the rho obtained scaling the quadrature point in the
              // [0,1] interval
              double rho = rho_jac_fact * rho_q_points[p](0);
              // this is the distance in the 3D space between quadrature point
              // and singularity
              Tensor<1, 3> R =
                inner_uv_q_points_spacedim[p] - eta_q_points_spacedim[0];
              double r = R.norm();
              // finally, this is the integral argument
              // the first term also needs the jacobian of the 3D mapping, which
              // was instead already included in the F_2 and F_1 factors of the
              // latter terms, and is therefore not needed there
              double fun = -1 / pow(r, 3) *
                           (3 * (R[2] / r) * (R * inner_uv_q_normals[p] / r) -
                            inner_uv_q_normals[p][2]);
              double arg = fun * rho * inner_face_fe_values.JxW(p) -
                           F_2 / pow(rho, 2) - F_1 / rho;

              //                  std::cout<<"rho: "<<rho<<"  r: "<<r<<"  arg1:
              //                  "<<1/pow(r,3)*rho*inner_face_fe_values.JxW(p)<<
              //                  "   arg2: "<<-F_2/pow(rho,2)-F_1/rho<<"  diff:
              //                  "<<1/pow(r,3)*rho*inner_face_fe_values.JxW(p)-F_2/pow(rho,2)-F_1/rho<<std::endl;
              ...
              // we add the conribution of the present node to the surface
              // integral the inner weights are given by rho_fe_v.JxW(p) but the
              // jacobian is given by the scaling factor rho_jac_fact
