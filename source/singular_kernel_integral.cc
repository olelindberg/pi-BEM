  ...
  for (unsigned int f=0; f<GeometryInfo<2>::faces_per_cell; ++f)
      ...
      if (!(((csi_0(0) == 0) && (csi_0(1) == 0)) || ((csi_1(0) == 0) && (csi_1(1) == 0))))
          ...
          for (unsigned int q=0; q<theta_q_points.size(); ++q)
              ...
              for (unsigned int i=0; i<3; ++i)
                  {
                  // Taylor expansion of xi-yi in polar coordinates around eta is
...
template<>
Tensor<1,2> SingularKernelIntegral<2>::evaluate_free_term_b()
{
ExcNotImplemented();
Tensor<1,2> dummy;
return dummy;
...
template<>
Tensor<1,3> SingularKernelIntegral<3>::evaluate_free_term_b()
{
  ...
  // the product of the jacobian by the normal vector Jxn is the surface normal vector
  // let us obtain it in terms of the mapping derivatives
  // The outward normal vector is g^(e)(eta^(e)) in [2]
  Tensor<1,3> eta_g;
  ...
  Tensor<1,3> eta_n = eta_g/eta_g_norm;
    ...
    std::cout << "eta_n " << eta_n[0] << ", " << eta_n[1] << ", " << eta_n[2] << std::endl;
      ...
      if (!(((csi_0(0) == 0) && (csi_0(1) == 0)) || ((csi_1(0) == 0) && (csi_1(1) == 0))))
          ...
          for (unsigned int q=0; q<theta_q_points.size(); ++q)
              ...
              for (unsigned int i=0; i<3; ++i)
                  ...
                  r_rhorho[q][i] = eta_jacobian_grad[i][0][0]*pow(cos(theta),2) 
          ...
          for (unsigned int q=0; q<theta_q_points.size(); ++q)
              {
                    ...
                    b += tmp*r_rho[q]*eta_g_norm/std::pow(r_rho_norm[q],5.0)*theta_fe_v.JxW(q);
