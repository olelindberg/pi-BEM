...
{
  {
      ...
      if (global_refinement)
      ...
      else
        {
          ...
          computational_domain.refine_and_resize(
            computational_domain.pre_global_refinements, input_path);
    ...
    auto forces =
      bem_problem.pressure_force(boundary_conditions.get_pressure());
    std::vector<Point<dim>> elevation;
    bem_problem.free_surface_elevation(boundary_conditions.get_pressure(),
                                       elevation);

    std::cout << "area  : " << area << "\n";
    std::cout << "volume: " << volume[0] << ", " << volume[1] << ", "
              << volume[2] << "\n";
    ...
    std::string  force_filename =
      boost::filesystem::path(output_path).append("forces.csv").string();
    ...
    std::string filename = boost::filesystem::path(output_path)
                             .append(boundary_conditions.output_file_name)
                             .string();
