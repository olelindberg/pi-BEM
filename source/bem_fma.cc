                           ...
                           unsigned int                                          sing_quad_order)
{
  ...
  fma_dh                    = &input_dh;
  dirichlet_nodes =
    new const Vector<double>(input_sn); // per quadratura singolare e octree generator
  this_cpu_set.clear();
  this_cpu_set.set_size(fma_dh->n_dofs());
  this_cpu_set.add_indices(
    input_sn.locally_owned_elements()); // = new const
...
void BEMFMA<dim>::declare_parameters(ParameterHandler &prm)
{
  prm.enter_subsection("Octree Params");
  {
    ...
    prm.declare_entry("Maximum Number of Collocation Points per Childless Block",
                      "20",
                      Patterns::Integer());
  }
  ...
  add_parameter(
    prm, &tbb_granularity, "Granularity for TBB simple for cycles", "10", Patterns::Integer());
}
...
void BEMFMA<dim>::parse_parameters(ParameterHandler &prm)
{
  prm.enter_subsection("Octree Params");
  {
    ...
    max_num_nodes_per_block =
      prm.get_integer("Maximum Number of Collocation Points per Childless Block");
  }
  prm.leave_subsection();
...
void BEMFMA<dim>::direct_integrals()
{
  ...
  for (unsigned int i = 0; i < fma_dh->get_fe().dofs_per_cell; ++i)
    sing_quadratures.push_back(
      QTelles<dim - 1>(singular_quadrature_order, fma_dh->get_fe().get_unit_support_points()[i]));
  ...
  auto f_init_prec_childless_worker =
    [this](types::global_dof_index kk, InitPrecScratch &, InitPrecCopy &copy_data) {
      ...
      if (block1Nodes.size() > 0)
      {
        ...
        for (std::set<types::global_dof_index>::iterator pos = block1IntList.begin();
        ...
        for (types::global_dof_index i = 0; i < block1Nodes.size(); i++)
          if (this_cpu_set.is_element(block1Nodes[i]))
          {
            ...
            for (std::set<types::global_dof_index>::iterator pos = directNodes.begin();
                 pos != directNodes.end();
                 pos++)
            {
...

  {
    ...
    // proccesor.
    auto f_init_prec_level_worker = [this, &startBlockLevel, &level](types::global_dof_index jj,
                                                                     InitPrecScratch &,
      ...
      if (nodesBlk1Ids.size() > 0) // !!!CHECK, IT SEEMS TO BE USELESS
      {
        ...
        for (unsigned int subLevel = 0; subLevel < block1->NumNearNeighLevels(); subLevel++)
        {
          ...
          for (std::set<types::global_dof_index>::iterator pos = nonIntList.begin();
  ...
  double filling_percentage = double(init_prec_sparsity_pattern.n_nonzero_elements()) /
                              double(fma_dh->n_dofs() * fma_dh->n_dofs()) * 100.;
  pcout << init_prec_sparsity_pattern.n_nonzero_elements() << " Nonzeros out of "
        << fma_dh->n_dofs() * fma_dh->n_dofs() << ":  " << filling_percentage << "%" << std::endl;
          ...
          if (this->this_cpu_set.is_element(nodeIndex))
          {
            ...
            for (it = directQuadPoints.begin(); it != directQuadPoints.end(); it++)
            {
              ...
              copy_data.vec_local_dof_indices.push_back(
                std::vector<types::global_dof_index>(this->fma_dh->get_fe().dofs_per_cell));
              ...
              if (is_singular == false)
              {
                ...
                for (std::set<types::global_dof_index>::iterator pos = cellQuadPoints.begin();
                     ...
                     pos++)
                {
                  ...
                  const Tensor<1, dim> R =
                    this->quadPoints.at(cell)[*pos] - support_points[nodeIndex];
                  ...
                  for (unsigned int j = 0; j < this->fma_dh->get_fe().dofs_per_cell; ++j)
                  {
                    copy_data.vec_local_neumann_matrix_row_i.back()(j) +=
                      ((D * this->quadNormals.at(cell)[*pos]) *
                       this->quadShapeFunValues.at(cell)[*pos][j] * this->quadJxW.at(cell)[*pos]);
                    copy_data.vec_local_dirichlet_matrix_row_i.back()(j) +=
                      (s * this->quadShapeFunValues.at(cell)[*pos][j] *
              ...
              else
              {
                ...
                const Quadrature<dim - 1> *singular_quadrature =
                  (dim == 2 ?
                     dynamic_cast<Quadrature<dim - 1> *>(&sing_quadratures[singular_index]) :
                     (dim == 3 ?
                        dynamic_cast<Quadrature<dim - 1> *>(&sing_quadratures[singular_index]) :
                        0));
                ...
                FEValues<dim - 1, dim> fe_v_singular(*(this->fma_mapping),
                                                     this->fma_dh->get_fe(),
                                                     *(singular_quadrature),
    ...
    if (copy_data.vec_node_index.size() > 0)
    {
      ...
      for (types::global_dof_index ii = 0; ii < copy_data.vec_node_index.size(); ++ii)
      {
        ...
        for (types::global_dof_index kk = foo_start; kk < foo_end; ++kk)
        {
          for (unsigned int j = 0; j < this->fma_dh->get_fe().dofs_per_cell; ++j)
          {
            this->prec_neumann_matrix.add(copy_data.vec_node_index[ii],
                                          copy_data.vec_local_dof_indices[kk][j],
                                          copy_data.vec_local_neumann_matrix_row_i[kk](j));
            this->prec_dirichlet_matrix.add(copy_data.vec_node_index[ii],
                                            copy_data.vec_local_dof_indices[kk][j],
            ...
            if ((*(this->dirichlet_nodes))(copy_data.vec_local_dof_indices[kk][j]) > 0.8)
            {
              ...
              this->init_preconditioner.add(copy_data.vec_node_index[ii],
                                            copy_data.vec_local_dof_indices[kk][j],
                                            -copy_data.vec_local_dirichlet_matrix_row_i[kk](j));
            }
            else
        ...
        if (this->this_cpu_set.is_element(nodeIndex)) //(m2l_flags[level][jj]==this_mpi_process)
        {
          ...
          for (it = directQuadPoints.begin(); it != directQuadPoints.end(); it++)
          {
                 ...
                 pos++)
            {
              ...
              {
                copy_data.vec_local_neumann_matrix_row_i.back()(j) +=
                  ((D * this->quadNormals[cell][*pos]) * this->quadShapeFunValues[cell][*pos][j] *
                   this->quadJxW[cell][*pos]);
                copy_data.vec_local_dirichlet_matrix_row_i.back()(j) +=
                  (s * this->quadShapeFunValues[cell][*pos][j] * this->quadJxW[cell][*pos]);
...
void BEMFMA<dim>::multipole_integrals()
{
  ...
  struct MultipoleData
  {
    ...
    std::map<types::global_dof_index, std::map<cell_it, std::vector<MultipoleExpansion>>>
  ...
  auto f_worker_multipole_integral =
    [this, dofs_per_cell](std::vector<types::global_dof_index>::iterator blocky,
                          MultipoleScratch &,
                          MultipoleData &copy_data) //, const unsigned int dofs_per_cell)
  {
    ...
    for (it = blockQuadPointsList.begin(); it != blockQuadPointsList.end(); it++)
    {
      ...
      for (types::global_dof_index k = 0; k < cellQuadPoints.size(); ++k)
      {
        ...
        for (unsigned int j = 0; j < this->fma_dh->get_fe().dofs_per_cell; ++j)
        {
          copy_data.myelemMultipoleExpansionsKer1[blockId][cell][j].AddNormDer(
            this->quadShapeFunValues.at(cell)[q][j] * this->quadJxW.at(cell)[q] / 4 / numbers::PI,
  ...
  auto f_copier_multipole_integral = [this](const MultipoleData &copy_data) {
    this->elemMultipoleExpansionsKer1.insert(copy_data.myelemMultipoleExpansionsKer1.begin(),
                                             copy_data.myelemMultipoleExpansionsKer1.end());
    this->elemMultipoleExpansionsKer2.insert(copy_data.myelemMultipoleExpansionsKer2.begin(),
...
// template <>
// void
// BEMFMA<2>::generate_multipole_expansions(const TrilinosWrappers::MPI::Vector &,
  ...
  const TrilinosWrappers::MPI::Vector &dphi_dn_values_in) const
{
  ...
  auto f_childless_creation_tbb =
    [this, &phi_values, &dphi_dn_values](blocked_range<types::global_dof_index> r) {
        ...
        for (it = blockQuadPointsList.begin(); it != blockQuadPointsList.end(); it++)
        {
          ...
          for (unsigned int jj = 0; jj < fma_dh->get_fe().dofs_per_cell; ++jj)
          {
            ...
            blockMultipoleExpansionsKer1.at(blockId).Add(
              elemMultipoleExpansionsKer1[blockId][cell][jj], phi_values(my_local_dof_indices[jj]));
          }
        } // end loop ond block elements
      }
    };

  parallel_for(blocked_range<types::global_dof_index>(0, childlessList.size(), tbb_granularity),
  ...
  auto f_worker_ascend = [this](typename std::vector<OctreeBlock<dim> *>::iterator block_it,
                                AscendScratchData &,
                                               ...
                                               TrilinosWrappers::MPI::Vector &matrVectProdD) const
{
  ...
  parallel_for(blocked_range<types::global_dof_index>(0, num_blocks, tbb_granularity),
  ...
  auto f_worker_Descend =
    [this, &support_points](std::vector<types::global_dof_index>::const_iterator block_it_id,
                            DescendScratchData &,
               ...
               pos1++)
          {
                 ...
                 ii++) // loop over each node of (*block_it)
            {
              ...
              copy_data.matrVectorProductContributionKer1(ii) +=
                this->blockMultipoleExpansionsKer1[block2Id].Evaluate(nodeBlk1);
              copy_data.matrVectorProductContributionKer2(ii) +=
                this->blockMultipoleExpansionsKer2[block2Id].Evaluate(nodeBlk1);
  ...
  */
  ...
  // parallelisation.
  auto f_local_evaluation_tbb = [this, &matrVectProdD, &matrVectProdN, &support_points](
                                ...
                                AffineConstraints<double> &          c) // TO BE CHANGED!!!
{
  ...
  final_prec_sparsity_pattern.reinit(this_cpu_set,
                                     mpi_communicator,
                                     (types::global_dof_index)125 * fma_dh->get_fe().dofs_per_cell);
  ...
  auto f_worker_prec = [this, &c](types::global_dof_index i, PrecScratch &, PrecCopy &copy_data) {
  ...
  parallel_for(blocked_range<types::global_dof_index>(0, fma_dh->n_dofs(), tbb_granularity),
...
void BEMFMA<dim>::compute_geometry_cache()
{
  ...
  cell_it gradient_cell = gradient_dh.begin_active(), gradient_endc = gradient_dh.end();
  ...
  for (cell = fma_dh->begin_active(); cell != endc; ++cell)
  {
    ...
    for (unsigned int j = 0; j < fma_dh->get_fe().dofs_per_cell; ++j)
    {
      ...
      for (std::set<types::global_dof_index>::iterator pos = duplicates.begin();
           pos != duplicates.end();
           pos++)
      {
...
void BEMFMA<dim>::generate_octree_blocking()
{
  ...
  types::global_dof_index maxNumBlocks =
    num_octree_levels * fma_dh->get_triangulation().n_active_cells() * fe_v.n_quadrature_points;
...

  {
...

    {
      ...
      if (dim == 3)
      {
        ...
        children[6] =
          new OctreeBlock<dim>(level, jj, pMin + Point<dim>(delta, delta, delta), delta);
    ...
    pcout << " Total nodes at level " << level << " of " << num_octree_levels << " are "
          << nodesCheck << std::endl;
    pcout << " Total quad points at level " << level << " of " << num_octree_levels << " are "
          << quadPointsCheck << std::endl;
    pcout << " Blocks at level " << level << " of " << num_octree_levels << " are "
          << endLevel[level] - endLevel[level - 1] << std::endl;
    pcout << " Total blocks at level " << level << " of " << num_octree_levels << " are "
          << endLevel[level] + 1 << std::endl;
    pcout << std::endl;
...

  {
...

    {
      ...
      // neighbors children
      for (std::set<types::global_dof_index>::iterator pos = parentNNeighs.begin();
           pos != parentNNeighs.end();
           pos++)

      {
        if (blocks[*pos]->GetBlockChildrenNum() == 0) // if a parent's near neigh is childless, he
                                                      // can be a near neigh: let's check
        {
      ...
      if ((block1->GetBlockChildrenNum() == 0)) // if the block is childless we must compute now its
                                                // nearneigh at all residual levels
      {
        ...
        block1->SetNonIntListSize(num_octree_levels - level +
...

        {
          ...
          for (std::set<types::global_dof_index>::iterator pos = upperLevelNNeighs.begin();
               pos != upperLevelNNeighs.end();
               pos++)

          {
            if (blocks[*pos]->GetBlockChildrenNum() == 0) // if nearneigh is childless, it will stay
                                                          // a near neigh
...

  {
         ...
         jj++) // loop over blocks of each level
    {
      ...
      for (unsigned int subLevel = 0; subLevel < block1->NumNearNeighLevels(); subLevel++)
      {
        ...
        for (std::set<types::global_dof_index>::iterator pos1 = NNList.begin();
             pos1 != NNList.end();
                       ...
                       // list (to account for childless blocks)
      {
        ...
        if (subLevel == 0) // if a block is childless we get its intList at the
        ...
        for (std::set<types::global_dof_index>::iterator pos1 = parentIntList.begin();
             pos1 != parentIntList.end();
             pos1++) // loop over blocks in parentIntList
        {
          OctreeBlock<dim> *block2 = blocks[*pos1];
          if (block2->GetBlockChildrenNum() == 0) // if blocks in parentIntList are childless, don't
                                                  // look for their children, but see if they are in
                                                  // nonIntList
          {
          ...
          {
            for (types::global_dof_index kk = 0; kk < block2->GetBlockChildrenNum();
                       ...
                       // parentIntList
            {
              ...
              if (intList.count(block2->GetChildId(kk)) == 0) // if these blocks are not in intList
                block1->AddBlockToNonIntList(subLevel,
                                             block2->GetChildId(kk)); // then they go in nonIntList
