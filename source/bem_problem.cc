                  ...
                  if (dim == 3)
                    {
                      ...
                      double sorter = acos(previous_proj_normal *
                                           unique_projected_normals[p]);
                    ...
                    {
                      // double sorter =
                      // average_normal*cross_product_2d(unique_ordered_normals[0]);
                    }
                }
        ...
        if (this_cpu_set.is_element(i))
          {
            ...
            for (types::global_dof_index j = start_helper;
                 j < std::min(
                       (types::global_dof_index)(i + preconditioner_band / 2),
      ...
      for (unsigned int local_id = 0; local_id < fe->dofs_per_cell; ++local_id)
        {
          ...
          pcout << free_term_b_all[global_id][0] << " "
                << free_term_b_all[global_id][1] << " "
                << free_term_b_all[global_id][2] << std::endl;
        }
    }
      ...
      if (this_cpu_set.is_element(i))
        {
          ...
          rhs[0] = vector_hyp_gradients_solution(i + 0 * dh.n_dofs()) -
                   0.0 * free_term_b_all[i][0] * phi_local[i];
          rhs[1] = vector_hyp_gradients_solution(i + 1 * dh.n_dofs()) -
                   0.0 * free_term_b_all[i][1] * phi_local[i];
          rhs[2] = vector_hyp_gradients_solution(i + 2 * dh.n_dofs()) -
                   0.0 * free_term_b_all[i][2] * phi_local[i];
