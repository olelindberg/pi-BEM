...
main (int argc, char *argv[])
{
  try
  {
    ...
    std::string pname  = boost::filesystem::path(options.get_input_path ()).append("parameters_bem_" + std::to_string (DEAL_II_DIMENSION) + ".prm").string();
    std::string pname2 = boost::filesystem::path(output_path).append("used_parameters_bem_" + std::to_string (DEAL_II_DIMENSION) + ".prm").string();
